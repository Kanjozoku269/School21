...
START_TEST(s21_nums4_f) {
  char s21_buffer[256] = {0};
  char buffer[256] = {0};
  s21_sprintf(s21_buffer, "%01.6f %1.0f %-1.0f", 123.212, 3.4414414,
              123.1312321);
  sprintf(buffer, "%01.6f %1.0f %-1.0f", 123.212, 3.4414414, 123.1312321);
}
END_TEST
...
START_TEST(s21_nums5_f) {
  char s21_buffer[256] = {0};
  char buffer[256] = {0};
  s21_sprintf(s21_buffer, "|%e| %.3g %E %e %*.3e %g %4.10g", 0.000000000078,
              0.0000000000234, 0.000013, 0, 00000242424, 1.0042311201,
              0.0000000465, 0.000465);
  sprintf(buffer, "|%e| %.3g %E %e %*.3e %g %4.10g", 0.000000000078,
          0.0000000000234, 0.000013, 0, 00000000242424, 1.0042311201,
          0.0000000465, 0.000465);
}
END_TEST
...
START_TEST(s21_nums6_f) {
  char s21_buffer[256] = {0};
  char buffer[256] = {0};
  s21_sprintf(s21_buffer, "%x %X %u %o %p", 412325, 12785231, 4841292929,
              919128381923, "qewqweqwe");
  sprintf(buffer, "%x %X %u %o %p", 412325, 12785231, 4841292929, 919128381923,
          "qewqweqwe");
}
END_TEST
...
START_TEST(s21_nums7_f) {
  char s21_buffer[256] = {0};
  char buffer[256] = {0};
  s21_sprintf(s21_buffer, "%10.3d   Hello   %d", 412313, -124123125123);
  sprintf(buffer, "%10.3d   Hello   %d", 412313, -124123125123);
}
END_TEST

START_TEST(s21_nums8_f) {
  char s21_buffer[256] = {0};
  char buffer[256] = {0};
  s21_sprintf(s21_buffer, "%5hd   %10.3d   Hello   %d %hd ", 123125151255,
              412313, -124123125123, 974654556545646542);
  sprintf(buffer, "%5hd  %10.3d   Hello   %d %hd ", 123125151255, 412313,
          -124123125123, 974654556545646542);
}
END_TEST

START_TEST(s21_nums9_f) {
  char s21_buffer[256] = {0};
  char buffer[256] = {0};
  s21_sprintf(s21_buffer, "% 10.6s", "DAdadadadad");
  sprintf(buffer, "% 10.6s", "DAdadadadad");
}
END_TEST

START_TEST(s21_nums10_f) {
  char s21_buffer[256] = {0};
  char buffer[256] = {0};
  s21_sprintf(s21_buffer, "% +-10d % +.13f", 1313999123, -1253.9123);
  sprintf(buffer, "% +-10d % +.13f", 1313999123, -1253.9123);
}
END_TEST

START_TEST(s21_nums11_f) {
  char s21_buffer[256] = {0};
  char buffer[256] = {0};
  s21_sprintf(s21_buffer, "|%100.23d| <%ld>", 1313999123, 848484848442313);
  sprintf(buffer, "|%100.23d| <%ld>", 1313999123, 848484848442313);
}
END_TEST
...
START_TEST(s21_nums12_f) {
  ...
  sprintf(buffer, "%1.3g %01g%# *.*g %n", 10009000., 0.003, -10, 5,
          0.000074047471, &w);
  s21_sprintf(s21_buffer, "%1.3g %01g %# *.*g %n", 10009000., 0.003, -10, 5,
              0.000074047471, &l);
}
END_TEST
...
Suite *lib_suite(void) {
  ...
  tc_strerror = tcase_create("strerror func");
  suite_add_tcase(s, tc_strerror);
  tcase_add_test(tc_strerror, s21_strerror_2);
  ...
  tc_nums_4 = tcase_create("f check");
  suite_add_tcase(s, tc_nums_4);
  tcase_add_test(tc_nums_4, s21_nums4_f);

  tc_nums_5 = tcase_create("f check");
  suite_add_tcase(s, tc_nums_5);
  tcase_add_test(tc_nums_5, s21_nums5_f);

  tc_nums_6 = tcase_create("f check");
  suite_add_tcase(s, tc_nums_6);
  tcase_add_test(tc_nums_6, s21_nums6_f);

  tc_nums_7 = tcase_create("f check");
  suite_add_tcase(s, tc_nums_7);
  tcase_add_test(tc_nums_7, s21_nums7_f);

  tc_nums_8 = tcase_create("f check");
  suite_add_tcase(s, tc_nums_8);
  tcase_add_test(tc_nums_8, s21_nums8_f);

  tc_nums_9 = tcase_create("f check");
  suite_add_tcase(s, tc_nums_9);
  tcase_add_test(tc_nums_9, s21_nums9_f);

  tc_nums_10 = tcase_create("f check");
  suite_add_tcase(s, tc_nums_10);
  tcase_add_test(tc_nums_10, s21_nums10_f);

  tc_nums_11 = tcase_create("f check");
  suite_add_tcase(s, tc_nums_11);
  tcase_add_test(tc_nums_11, s21_nums11_f);

  tc_nums_12 = tcase_create("f check");
